...
async def main():
    ...
    async with agent.run_mcp_servers():
        ...
        print("\n5. Пошук BMW з ціною 10-50 тис. USD:")
        result = await agent.run(
            'Знайди BMW з ціною від 10000 до 50000 USD, покажи перші 3 результати'
        )
        print(result.output)
        ...
        print("\n8. Якщо знайдено авто, отримуємо детальну інформацію:")
        result = await agent.run(
            'Якщо в попередніх результатах є ID авто, покажи детальну інформацію про перше з них'
        )
        print(result.output)
